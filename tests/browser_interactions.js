...
async function main(){const c=await launch({dir:OUT,name:'interactions',base});let current;
 ...
 async function measuredSvg(){
  const plan=await c.evaluate(`(()=>{const all=[...document.querySelectorAll('svg')],batches=[];let indexes=[],labels=0,semanticLabels=0,semanticSvgs=0,largest=0;const flush=()=>{if(indexes.length){batches.push({indexes,labels});indexes=[];labels=0}};for(const [index,svg] of all.entries()){if(!activeContent(svg))continue;const count=[...svg.querySelectorAll('text')].filter(e=>semanticSvgText(e)&&e.ownerSVGElement===svg).length;if(!count)continue;semanticSvgs++;semanticLabels+=count;largest=Math.max(largest,count);if(indexes.length&&labels+count>8)flush();indexes.push(index);labels+=count;if(labels>=8)flush()}flush();return{documentSvgs:all.length,semanticSvgs,semanticLabels,largest,batches}})()`);
  if(plan.largest>48)throw Error('raster setup failure: one SVG exceeds bounded semantic label batch '+JSON.stringify(plan));
  const merged={rows:[],capabilities:[],failures:[],batchPlan:plan};
  for(const batch of plan.batches){const part=await c.evaluate(`svgGeometry([...document.querySelectorAll('svg')].filter((_svg,index)=>${JSON.stringify(batch.indexes)}.includes(index)))`);merged.rows.push(...part.rows);merged.capabilities.push(...part.capabilities);merged.failures.push(...part.failures);}
  const measuredLabels=merged.capabilities.reduce((n,row)=>n+row.semantic,0);
  if(merged.capabilities.length!==plan.semanticSvgs||measuredLabels!==plan.semanticLabels)throw Error('raster setup failure: SVG batch inventory changed '+JSON.stringify({plan,measuredSvgs:merged.capabilities.length,measuredLabels}));
  return merged;
 }
 async function snapshot(name,details={},selector){const targetsResult=await c.evaluate('targets()'),svgResult=await measuredSvg(),rootResult=await c.evaluate('rootLayout()'),measured={targets:targetsResult,svg:svgResult,root:rootResult};const failures=[...measured.targets.failures,...measured.svg.failures,...measured.root.failures,...c.events.exceptions,...c.events.console,...c.events.logs,...c.events.blocked];const row={name,...current,...details,...measured,failures};rows.push(row);fs.appendFileSync(path.join(OUT,'observations.jsonl'),JSON.stringify(row)+'\n');if(selector)await c.evaluate(`document.querySelector(${JSON.stringify(selector)}).scrollIntoView({block:'center',behavior:'instant'})`);else await c.evaluate('window.scrollTo({top:0,left:0,behavior:"instant"})');await c.evaluate('new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))');const png=await c.send('Page.captureScreenshot',{format:'png',captureBeyondViewport:false});fs.writeFileSync(path.join(OUT,name+'-'+current.width+'.png'),Buffer.from(png.data,'base64'));assert.equal(failures.length,0,name+': '+JSON.stringify(failures.slice(0,3)));}
