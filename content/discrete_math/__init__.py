...
PATH = {
    ...
    "sequence_intro": (
        'Each course assumes the ones before it and nothing else. Induction and Recursion proves things about the sets built in Sets, Relations, and Functions; Discrete Probability counts the outcomes Combinatorics and Counting taught you to count; Algorithms and Complexity analyses the structures Graphs and Trees built.'
    ),
    "why_order": ['Logic comes first because everything after it is a claim that has to be stated precisely enough to be true or false. A reader who has not settled what `∀x ∃y P(x, y)` means cannot read a single theorem in the later courses, and will silently misread several.', 'Sets, relations and functions come second because they are the vocabulary. An equivalence relation, a bijection and a partial order each appear in four of the six courses that follow; they are introduced once.', 'Induction is third rather than last because it is the proof technique the rest of the path depends on. Counting arguments, recurrence solutions, graph theorems and algorithm correctness are all induction wearing different clothes.', 'Counting, probability and number theory then apply that machinery to three specific worlds, and graphs give it an object to act on. Algorithms and complexity comes last because it needs all of them: recurrences from Induction and Recursion, counting from Combinatorics and Counting, and graphs from Graphs and Trees.'],
    "prerequisites": ['School algebra, and nothing beyond it. You need to be comfortable rearranging an equation, working with exponents, and reading summation notation &mdash; the first course that uses `Σ` explains it where it first appears.', 'No calculus. Nothing on this path takes a limit, a derivative or an integral. Where a growth rate is compared to another, it is compared by an explicit constant and threshold rather than by a limit.', 'No programming. Algorithms and Complexity reads pseudocode and Number Theory and Cryptography walks through algorithms step by step, but nothing asks you to write or run code. The labs execute in your browser so that you can watch an algorithm rather than implement it.', 'Patience with definitions. The single largest difficulty in this subject is that its words are used precisely: <em>or</em> is inclusive, <em>some</em> means at least one, and a <em>graph</em> is not a plot. Every one of those is stated where it first matters.'],
